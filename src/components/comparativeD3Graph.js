    ...
    if (currentData !== null && historyData !== null) {
      ...
      const dataForRender = indexArray.map(index => {
        ...
        if (index < todayDataLength) {
          ...
          if (index < todayInHistoryData.observations.length) {
            if (
              ...
              historyTemperature = Number(
                todayInHistoryData.observations[index].tempi
              );
            } else if (
              ...
              historyTemperature = Number(
                todayInHistoryData.observations[index - 1].tempi
              );
            } else {
          ...
          if (followingDayHour < tomorrowInHistoryData.observations.length) {
            if (
              tomorrowInHistoryData.observations[followingDayHour] !==
                undefined &&
              Number(
                tomorrowInHistoryData.observations[followingDayHour].tempi > 0
              )
            ) {
              historyTemperature = Number(
                tomorrowInHistoryData.observations[followingDayHour].tempi
              );
            } else if (
              Number(
                tomorrowInHistoryData.observations[followingDayHour - 1].tempi
              ) > 0 &&
              Number(
                tomorrowInHistoryData.observations[followingDayHour + 1].tempi
              ) > 0
              ...
              historyTemperature =
                (Number(todayInHistoryData.observations[index - 1].tempi) +
                  Number(todayInHistoryData.observations[index + 1].tempi)) /
                2;
            } else {
        ...
        return {
          todaysTemperature: Number(
            currentData.hourly_forecast[index].temp.english
          ),
      ...
      const d3ParseScale = d3
        .scaleTime()
        .domain([
          d3.timeParse("%Y-%m-%dT%H:%M:%S")(today),
          d3.timeParse("%Y-%m-%dT%H:%M:%S")(tomorrow)
        ])
        .range([30, WIDTH]);
      ...
      //create path drawers
      const generateCurrentTemperatureLine = d3
        .line()
        .x(data => xScale(data.time))
        .y(data => yScale(data.todaysTemperature))
        .curve(d3.curveCardinal);

      const generateHistoryTemperatureLine = d3
        .line()
        .x(data => xScale(data.time))
      ...
      vis
        ...
        .attr("d", area.y0(data => yScale(data.historyTemperature)))
      ...
      vis.append("path").attr("class", "line").attr("d", line);
